...
public class FetchDataIntoDifferentURL {
    ...
    public void FetchDataIntoDifferentURL(List list ,List jpgList) {
        //设置数据库
...
//                Document document = Jsoup.connect("http://www.tiantangbbs.com/thread-278909-1-1.html")  //用作测试
                    ...
                    //给movieJPG赋值
                    movieJPG = jpgList.get(i).toString();
                    //给种子页面赋值
                    movieDownload = list.get(i).toString();
                    //将字符串解析
...
//                System.exit(1);  //测试只执行一次
                    ...
                    sql = "INSERT INTO movie_data" +
                            "(name,picture,classfication,area,year,director,star,score,summary,download,jpg) " +
                            "VALUES('"+movieName
                            +"','"+moviePicture
                            +"','"+movieClassification
                            +"','"+movieArea
